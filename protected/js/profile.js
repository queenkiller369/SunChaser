...
async function loadUser() {
    ...
    username.innerHTML =`<h1>${realUser.username}</h1>`
}
...
async function loadUserPhotos() {
  ...
  const res = await fetch(`/userPhotos`);
    ...
    recentPhotoContainer.innerHTML += `
        <div class='col col-sm-4 j photo-card' data-id="${recentPhoto.id}">
          <div class="media-card">
            <h3 data-id="${recentPhoto.id}">${recentPhoto.title}</h3>
          ...
          modalContent.innerHTML = `
                ...
                <div class="modal-body">
                  ...
                  <ul class="photo-details">
                    ...
                    <h6 data-id="${targetId}">Description: ${result.description}</h6>
                    <h6 data-id="${targetId}">District: ${result.district}</h6>
                    <h6 data-id="${targetId}">Location: Latitude--${result.location}</h6>
                    <h6 data-id="${targetId}">Date & Time: ${result.updated_at.substring(0, 19).replace("T", "  ")}</h6>
                    <h6 data-id="${targetId}">Photograph by Username: ${result.username}</h6>
                    <form>
      ...
      if (event.target.matches(".j")) {
        ...
        modalContent.innerHTML = `
          ...
          result.description
                    ...
                    <h6 data-id="${targetId}">District: ${result.district}</h6>
                    <h6 data-id="${targetId}">Location: ${result.location}</h6>
                    <h6 data-id="${targetId}">Date & Time: ${result.updated_at
            .substring(0, 19)
            .replace("T", "  ")}</h6>
                    <h6 data-id="${targetId}">Photograph by Username: ${
          result.username
          }</h6>
                    <form>
                      <div class="form-comment${recentPhoto.id}">
